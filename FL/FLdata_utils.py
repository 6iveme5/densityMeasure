...
def load_and_clean_data(data_dir=DATA_DIR, save_split=True):
    def _read_csv(path):
        return pd.read_csv(path)

    def _clean_dataframe(df, keep_unit=False):
        df = df.copy()
        # 日期解析
        ...
        drop_cols = [
            ...
            'requestType', 'requestBedType', 'admissionBedType'
            # 不删admissionHealthUnit, 等分完组再删
        ]
        if not keep_unit:
            drop_cols.append('admissionHealthUnit')

        df.drop(columns=[col for col in drop_cols if col in df.columns], inplace=True)

        # 字符串列编码，跳过 admissionHealthUnit
        for col in df.select_dtypes(include='object').columns:
            if col != 'admissionHealthUnit':
    ...
    # 保留admissionHealthUnit用于分组
    train_raw = pd.read_csv(train_path)
    val_raw = pd.read_csv(val_path)

    train_df = _clean_dataframe(train_raw, keep_unit=True)
    val_df = _clean_dataframe(val_raw, keep_unit=True)
    ...
    # --- 自动分组并按实体名称保存 ---
    if save_split and 'admissionHealthUnit' in train_df.columns:
        fl_dir = os.path.join(data_dir, 'FLData')
        os.makedirs(fl_dir, exist_ok=True)

        unit_names = train_df['admissionHealthUnit'].dropna().unique()
        print("自动检测到的 admissionHealthUnit 单位名如下：")
        for name in unit_names:
            print(name)
            train_unit = train_df[train_df['admissionHealthUnit'] == name]
            # 用下划线等字符替换掉文件名中的空格、特殊字符
            safe_name = re.sub(r'[^\w\-_\.]', '_', str(name))
            unit_df = train_unit.drop(columns=['admissionHealthUnit'], errors='ignore')
            unit_df.to_csv(os.path.join(fl_dir, f'{safe_name}.csv'), index=False)
